...
- 左侧导航、顶部搜索、右侧图例和地图工具均完整显示且不遮挡。
- 建筑保持低矮，不遮挡道路；湖泊、绿地、运动场、广场和树木易于区分。
- 主路、支路和入口道路宽度不同，最终路径为橙色宽带。
- 左键和右键能分别选择起点、终点。
- `Enter` 能启动访问动画，右侧显示距离和访问节点数。
- `D` 与 `A` 的最短距离一致。
- `F1` / `F2` / `F3` 能切换导航、全览和路径视角。
- 右下角按钮能缩放、复位以及切换 2.5D/3D 展示。
- 方向键、滚轮、`Home` 和 `R` 工作正常。
- `Esc` 能关闭 3D 窗口并返回控制台。

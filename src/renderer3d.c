...
static int ui_latin_font_owned = 0;

static int font_has_codepoint(Font font, int codepoint) {
    int index;
...
static int try_load_ui_font(const char *path, const int *codepoints,
                            int codepoint_count) {
    Font candidate;
    Font fallback = GetFontDefault();
    if (path == NULL || path[0] == '\0' || !FileExists(path)) return 0;
    candidate = LoadFontEx(path, 32, (int *)codepoints, codepoint_count);
    if (IsFontValid(candidate) && font_has_codepoint(candidate, 0x4E2D)) {
        ui_font = candidate;
        ui_font_owned = candidate.texture.id != fallback.texture.id;
        SetTextureFilter(ui_font.texture, TEXTURE_FILTER_BILINEAR);
        return 1;
    }
    if (IsFontValid(candidate) && candidate.texture.id != fallback.texture.id) {
...
static void load_ui_font(void) {
    const char *system_root = getenv("WINDIR");
    const char *portable_candidates[] = {
        "/System/Library/Fonts/PingFang.ttc",
        "/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc",
        "/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc"
    };
    char windows_paths[3][512] = {{0}};
    char latin_path[512] = {0};
    int codepoint_count = 0;
    int *codepoints = LoadCodepoints(ui_font_glyphs(), &codepoint_count);
    int i;
    ui_font = GetFontDefault();
    ui_font_owned = 0;
    if (system_root == NULL || system_root[0] == '\0') system_root = getenv("SystemRoot");
    if (system_root != NULL && system_root[0] != '\0') {
        snprintf(windows_paths[0], sizeof(windows_paths[0]), "%s/Fonts/msyh.ttc", system_root);
        snprintf(windows_paths[1], sizeof(windows_paths[1]), "%s/Fonts/msyhl.ttc", system_root);
        snprintf(windows_paths[2], sizeof(windows_paths[2]), "%s/Fonts/simhei.ttf", system_root);
        for (i = 0; i < 3; ++i) {
            if (try_load_ui_font(windows_paths[i], codepoints, codepoint_count)) break;
        }
        snprintf(latin_path, sizeof(latin_path), "%s/Fonts/segoeui.ttf", system_root);
    }
    if (!ui_font_owned) {
        for (i = 0; i < (int)(sizeof(portable_candidates) /
                              sizeof(portable_candidates[0])); ++i) {
            if (try_load_ui_font(portable_candidates[i], codepoints, codepoint_count)) break;
        }
    }
    ui_latin_font = GetFontDefault();
    ui_latin_font_owned = 0;
    if (latin_path[0] != '\0' && FileExists(latin_path)) {
        Font candidate = LoadFontEx(latin_path, 32, NULL, 0);
        if (IsFontValid(candidate) &&
            candidate.texture.id != GetFontDefault().texture.id) {
            ui_latin_font = candidate;
            ui_latin_font_owned = 1;
            SetTextureFilter(ui_latin_font.texture, TEXTURE_FILTER_BILINEAR);
        }
    }
    UnloadCodepoints(codepoints);
}

static void unload_ui_font(void) {
    if (ui_latin_font_owned) UnloadFont(ui_latin_font);
    if (ui_font_owned) UnloadFont(ui_font);
    ui_font = (Font){0};
    ui_font_owned = 0;
    ui_latin_font = (Font){0};
    ui_latin_font_owned = 0;
}

static int text_is_ascii(const char *text) {
    const unsigned char *cursor = (const unsigned char *)text;
    while (cursor != NULL && *cursor != '\0') {
        if (*cursor >= 0x80) return 0;
        cursor++;
    }
    return 1;
}

static void draw_ui_text(const char *text, int x, int y, int font_size, Color color) {
    int use_latin = ui_latin_font_owned && text_is_ascii(text);
    Font font = use_latin ? ui_latin_font : ui_font;
    float display_size = (float)font_size * (use_latin ? 1.12f : 1.0f);
    DrawTextEx(font, text, (Vector2){(float)x, (float)y}, display_size, 0.0f, color);
}

static int measure_ui_text(const char *text, int font_size) {
    int use_latin = ui_latin_font_owned && text_is_ascii(text);
    Font font = use_latin ? ui_latin_font : ui_font;
    float display_size = (float)font_size * (use_latin ? 1.12f : 1.0f);
    return (int)ceilf(MeasureTextEx(font, text, display_size, 0.0f).x);
}

#define DrawText(text, x, y, size, ...) \
...
static const Place *selected_place(const PlaceStore *places, int place_id,
    ...
    if (place != NULL && (place->entrance_node_id == node_id ||
                          place->display_node_id == node_id)) {
        return place;
    }
    place = storage_find_place_by_display(places, node_id);
    return place != NULL ? place : storage_find_place_by_entrance(places, node_id);
}

static const Node *place_display_node(const Graph *graph, const PlaceStore *places,
...
static void draw_roads(const Graph *graph, const SceneLayout *layout,
    ...
    for (i = 0; i < graph->edge_count; ++i) {
        ...
        point_count = edge->geometry_count;
        for (point_index = 0; point_index + 1 < point_count; ++point_index) {
            Vector3 a = route_geometry_point(graph, edge, point_index,
                                             direction < 0, layout);
...
static void draw_selection_markers(const Graph *graph, const PlaceStore *places,
                                   ...
                                   const RendererState *state) {
    const Node *start = place_display_node(graph, places, state->start_place_id,
                                           state->start_id);
...
static void draw_labels(const Graph *graph, const PlaceStore *places,
    ...
    for (pass = 0; pass < 3; ++pass) {
        for (i = 0; i < graph->node_count; ++i) {
            const Node *node = &graph->nodes[i];
            const Place *start_place = storage_find_place(places, state->start_place_id);
            const Place *goal_place = storage_find_place(places, state->goal_place_id);
            int selected_priority = (start_place != NULL &&
            ...
            const Place *place = start_place != NULL && node->id == start_place->display_node_id
                                 ...
                                 : node->id == state->search_match_id
                                     ...
                                     : storage_find_place_by_display(places, node->id);
            int priority = selected_priority || place != NULL;
            int label_pass = selected_priority ? 0 : place != NULL ? 1 : 2;
            int font_size = selected_priority ? 16 : 14;
            const char *label = ui_place_name(place != NULL && place->name[0] != '\0'
                                                  ? place->name
                                                  : node->name);
            if (label_pass != pass ||
...
static void draw_navigation_panel(const Graph *graph, const PlaceStore *places,
    ...
    draw_text_fit(start ? ui_place_name(start_place ? start_place->name : start->name)
                        : ui_text(UI_START_HINT), 53, 123, 215, 15,
                  (Color){44, 63, 57, 255});
    DrawText(ui_text(UI_DESTINATION), 20, 169, 13, (Color){98, 111, 105, 255});
    draw_ui_card((Rectangle){20, 186, 260, 48}, (Color){252, 242, 239, 255},
                 (Color){235, 216, 211, 255});
    DrawCircle(38, 210, 6, (Color){221, 78, 69, 255});
    draw_text_fit(goal ? ui_place_name(goal_place ? goal_place->name : goal->name)
...
static int update_search(const PlaceStore *places, RendererState *state) {
    ...
    while (key > 0) {
        ...
        if (key >= 32) {
            ...
            if (utf8 != NULL && utf8_size > 0 &&
                length + (size_t)utf8_size < sizeof(state->search_query)) {
                memcpy(state->search_query + length, utf8, (size_t)utf8_size);
            ...
            if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) ||
                ...
                if (picked >= 0) {
                    ...
                    if (IsMouseButtonPressed(MOUSE_BUTTON_RIGHT)) {
                        state.goal_id = place != NULL ? place->entrance_node_id : picked;
